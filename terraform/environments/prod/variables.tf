...
variable "enable_deployment" {
  description = "PROD foundation deployment switch; Phase 1 requires the complete topology to remain disabled."
  type        = bool
  default     = false

  validation {
    condition     = !var.enable_deployment
    error_message = "PROD foundation must remain disabled throughout Phase 1."
  }
}
...
variable "monthly_budget_usd" {
  description = "Design-only Human review threshold for future PROD cost; this does not create an AWS Budgets resource."
  type        = number

  validation {
    condition     = var.monthly_budget_usd >= 12 && var.monthly_budget_usd <= 100
    error_message = "monthly_budget_usd must be at least USD 12 and no more than USD 100 without a new cost review."
  }
}

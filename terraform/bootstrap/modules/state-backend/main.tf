...
resource "aws_kms_key" "state" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(
      concat(var.allow_root_for_v1 ? [{ Sid = "EnableV1RootScopedAccess", Effect = "Allow", Principal = { AWS = "arn:aws:iam::${var.account_id}:root" }, Action = ["kms:DescribeKey", "kms:PutKeyPolicy", "kms:EnableKeyRotation", "kms:ScheduleKeyDeletion", "kms:CancelKeyDeletion", "kms:Encrypt", "kms:Decrypt", "kms:GenerateDataKey", "kms:ReEncryptFrom", "kms:ReEncryptTo"], Resource = "*" }] : [], [for role_index, role_arn in var.kms_admin_role_arns : {

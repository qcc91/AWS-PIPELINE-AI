...
- 2026-09-10: Human approved a shared BI/ML business-data design. The same customer, address, product, broker, policy, coverage, claim, claim item, payment, status, and interaction facts serve both uses. Silver remains business-oriented; ML features are point-in-time derivatives rather than a separate source. The V1 target is `high_risk_claim` at submission time with future outcome-derived labels and output `high_risk_probability`. The earlier four-row fraud fixture is only a smoke artifact and is not valid final V1 evidence.
- 2026-09-10: Human submitted the prepared Titan Text Embeddings V2 request through AWS Basic Support. Case `178899964200695` was created at `2026-09-10T00:20:42.712Z`, category `Service Quotas, General`, severity `General question`, initial status `Unassigned`. The account remains FREE and no Titan retry is authorized while effective RPM is zero.
- 2026-09-10: SageMaker quotas propagated above the requested minimum (Training 15, Transform 8). V1 ML completed from 120 Gold feature rows with one `ml.m5.large` per job, no endpoint/notebook, encrypted artifact retention, transient model deletion, Validation AUC 0.65556, and Test AUC 0.62222. An initial artifact-upload failure was fixed by explicitly passing the platform KMS key to Training/Transform outputs; the control-bucket deny policy was not weakened.
- 2026-09-10: AWS Support case `178899964200695` corrected the Titan Text Embeddings V2 diagnosis: actual backend limits are 6,000 RPM and 300,000 TPM, while the zero Service Quotas display is a known inconsistency. Earlier HTTP 429 responses were transient managed-ingestion throttling, not FREE-account entitlement failure; no quota increase is requested.
- 2026-09-10: V1 RAG completed without architecture or account-plan changes. After confirming no active ingestion, exactly one job `U0DDU3DXFT` completed in about four seconds, indexed two of two documents with zero failures, and produced two S3 Vectors entries. Three representative Nova Micro answers were grounded in the indexed documents and returned S3 citations. The CLI now makes document sync opt-in and refuses overlapping ingestion jobs.
